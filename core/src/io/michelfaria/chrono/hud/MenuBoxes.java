...
public final class MenuBoxes {
    ...
    This was the function used to make dynamic menu boxes.
    It was over-engineered, so ...

    public static Table makeBox(float width, float height) {
        ...
        // Add top-left
        TextureRegion topLeft = TxUtil.findRegion(Core.atlas, UI_BOX_0_TOP_LEFT);
        table.add(new Image(topLeft));

        // Add top
        TextureRegion top = TxUtil.findRegion(Core.atlas, UI_BOX_0_TOP);
        table.add(new Image(new TiledDrawable(top)));
        Cell topCell = table.getCells().peek();
        topCell.fill();

        // Add top-right
        TextureRegion topRight = TxUtil.findRegion(Core.atlas, UI_BOX_0_TOP_RIGHT);
        ...
        // Add left
        TextureRegion left = TxUtil.findRegion(Core.atlas, UI_BOX_0_LEFT);
        table.add(new Image(left));
        Cell leftCell = table.getCells().peek();
        leftCell.fill();

        // Add filling
        TextureRegion filling = TxUtil.findRegion(Core.atlas, UI_BOX_0_FILLING);
        table.add(new Image(new TiledDrawable(filling)));
        Cell fillingCell = table.getCells().peek();
        // Set width of the box
        fillingCell.width(width);
        // Set the height of the box
        fillingCell.height(height);
        // Make the center texture fill
        fillingCell.fill();

        // Add right
        TextureRegion right = TxUtil.findRegion(Core.atlas, UI_BOX_0_RIGHT);
        ...
        // Add bottom-left
        TextureRegion bottomLeft = TxUtil.findRegion(Core.atlas, UI_BOX_0_BOTTOM_LEFT);
        table.add(new Image(bottomLeft));

        // Add bottom
        TextureRegion bottom = TxUtil.findRegion(Core.atlas, UI_BOX_0_BOTTOM);
        table.add(new Image(bottom));
        Cell bottomCell = table.getCells().peek();
        bottomCell.fill();

        // Add bottom-right
        TextureRegion bottomRight = TxUtil.findRegion(Core.atlas, UI_BOX_0_BOTTOM_RIGHT);
